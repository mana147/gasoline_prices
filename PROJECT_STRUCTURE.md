...
- **Frontend**: HTML/CSS/JS tĩnh, giao diện tiếng Việt
...
│   ├── app.js                       # Express factory: middleware, mount routes
│   │
│   ├── config/
...
│   │
│   ├── controllers/
│   │   ├── auth.controller.js       # Handlers: login, logout, register, user CRUD
...
│
├── view/
│   ├── index.html                   # Dashboard chính (date picker → fetch → hiển thị phụ thu)
...
## Module chính: calculator_gasoline.js
...
2. **Bảng phụ thu hardcode** trong `calculator_gasoline.js` — muốn thay đổi mức giá phải sửa trực tiếp file đó
